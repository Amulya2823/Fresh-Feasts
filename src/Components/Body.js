...
const Body = () => {
    ...
    const fetchData = async() => {
        const data = await fetch("https://www.swiggy.com/dapi/restaurants/list/v5?lat=17.5171428&lng=78.315742&is-seo-homepage-enabled=true&page_type=DESKTOP_WEB_LISTING");
    ...
    return(
        <div >
            <div className='flex items-center justify-center'>
                ...
                onClick={ () => {
                    const filterbutton = listOfRestuarants.filter((res) => res?.info?.avgRating>=4)
                    setfilteredRestuarants(filterbutton);
                }}>Top Rated Restuarants</button>

                <div>
                    <input className=" w-72 h-10 border border-solid border-black" type="text" placeholder="Search for restuarant" 
                    value={searchtext} 
                    onChange={ (e) => {
                    ...
                    <button  className="px-4 py-2 rounded-md m-4 bg-green-300"
                    onClick={ () => {
                        const filteredlistofres = listOfRestuarants.filter((restuarant) => restuarant?.info?.name.toLowerCase().includes(searchtext.toLowerCase()));
            ...
            <div >
                <div className='flex flex-wrap'>
                    ...
                    {restaurant.info.isOpen ? (
                        <CardwithOfferlabel resData = {restaurant}/>
                    ):(
